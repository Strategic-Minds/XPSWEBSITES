...
export default function VizualXPage() {
  ...
  const [command, setCommand] = useState("white swirl epoxy floor");
  const [status, setStatus] = useState("Vizzy is ready");
  const [readout, setReadout] = useState("Upload a photo, then outline the floor or run auto-detect.");
  const [busy, setBusy] = useState(false);

  useEffect(() => {
    draw();
  }, [image, polygon, finish]);
  ...
  function upload(event: ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = () => {
      ...
      nextImage.onload = () => {
        ...
        setReadout("Tap the visible floor corners or run auto-detect.");
      };
  ...
  async function detectFloor() {
    ...
    try {
      ...
      setReadout(`${result.mode === "ai" ? "AI detected" : "Estimated"} floor. ${result.notes || ""}`);
      setStatus("Floor outlined");
      setBusy(false);
      return true;
    } catch (error) {
      setStatus(error instanceof Error ? error.message : "Outline failed");
  ...
  async function searchStyles() {
    ...
    try {
      ...
      setReadout("Choose a card, then render or download the local preview.");
      setBusy(false);
      return options;
    } catch {
  ...
  async function renderAiPreview() {
    ...
    try {
      ...
      setReadout("The local preview still works. Add OpenAI environment variables in Vercel for final AI rendering.");
    }
  ...
  return (
    <main className="vx-page">
      <header className="vx-topbar">
        <a className="vx-brand" href="/" aria-label="Back to Phoenix Epoxy Pros">
          <span className="vx-brand-mark" />
          <span>
            <strong>Vizual-X</strong>
            <small>XPS floor visualizer</small>
          </span>
        </a>
      ...
      <section className="vx-hero">
        <div>
          <p className="vx-eyebrow">Upload. Outline. Preview.</p>
          <h1>See the floor finish before you send the estimator package.</h1>
          <p>
            Upload a room or floor photo, outline the floor area, test a finish direction, then download the preview
            and submit it through the 15% Digital Estimator.
          </p>
        </div>
        <div className="vx-vizzy-card">
          ...
          <p>Tip: use the color charts on the homepage first, then enter the exact color name in your estimator form.</p>
        </div>
      </section>
      ...
      <section className="vx-app-grid">
        <div className="vx-canvas-shell">
          ...
          {!image && (
            <div className="vx-empty">
              ...
              <strong>Then tap the floor corners or let Vizzy estimate the floor area.</strong>
            </div>
          )}
        </div>
        ...
        <aside className="vx-side">
          ...
          <section className="vx-panel vx-command-box">
            <h2>Style idea</h2>
            <p>Describe the look you want, or use the default white swirl epoxy direction.</p>
            <textarea value={command} onChange={(event) => setCommand(event.target.value)} />
            <div className="vx-row">
              <button onClick={searchStyles} disabled={busy}>Find styles</button>
              <button className="vx-magic" onClick={runFullFlow} disabled={busy}>Do it all</button>
            </div>
          </section>

          <section className="vx-panel">
            <h2>Choose finish</h2>
            <p>Pick a reference card. The canvas preview updates when the floor is outlined.</p>
          ...
          <section className="vx-panel">
            ...
            <p>{readout}</p>
            <div className="vx-row">
              <button onClick={detectFloor} disabled={busy}>Outline floor</button>
              <button className="vx-primary" onClick={renderAiPreview} disabled={busy}>Render floor</button>
            </div>
            <button onClick={downloadPreview}>Download preview</button>
            <a className="vx-submit-link" href="/#digital-estimator">Use 15% Digital Estimator</a>
          </section>
        </aside>
      </section>
    </main>
