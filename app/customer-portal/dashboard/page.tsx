...
const STEP_LABELS = [
  ...
  { num: 2, label: "Proposal Sent",    sub: "Review and approve"   },
...
export default function ClientDashboardPage() {
  ...
  if (loading) {
    return (
      <DashboardShell role="client" roleLabel="Portal" user="Loading..." active="My Project">
  ...
  return (
    <DashboardShell role="client" roleLabel="Portal" user={`${firstName} — Client Portal`} active="My Project">
      ...
      {/* WELCOME BANNER */}
      <div className="ds-card" style={{ background: "linear-gradient(135deg,#0a0a0a,#111)", border: "1px solid #f6b800" }}>
        <div className="ds-card-body" style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 20, alignItems: "center" }}>
          <div>
            ...
            <h1 style={{ margin: "0 0 8px", fontSize: "1.4rem", fontWeight: 900 }}>Hey {firstName} 👋</h1>
            <p style={{ margin: "0 0 14px", fontSize: 13, color: "#888" }}>
              ...
              {currentStatus === "proposal_sent" && "Your proposal is ready — review it and approve to lock your install date."}
              {currentStatus === "deposit_paid" && "Deposit received! Your install is scheduled. Get ready for a new floor."}
              {currentStatus === "in_progress" && "Your install is happening today! Our crew is on site right now."}
              {currentStatus === "scheduled" && "Your install date is confirmed. We will see you soon!"}
              {currentStatus === "completed" && "Your floor is done! Thank you for choosing Phoenix Epoxy Pros."}
            </p>
            <a href="https://wa.me/17722090266" target="_blank" rel="noopener noreferrer" className="ds-btn gold" style={{ fontSize: 13 }}>
      ...
      <div className="ds-kpi-row">
        {[
          ...
          { label: "Your Discount", value: "15% OFF",                                         sub: "Digital bid savings" },
          { label: "Install Date",  value: (job?.install_date as string)   || "TBD",          sub: "Confirmed date"   },
        ].map(({ label, value, sub }) => (
        ...
        <div className="ds-card-body">
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10 }}>
            {COLOR_CHARTS.map(chart => (
              <div
                key={chart.id}
                onClick={() => setActiveColor(chart.title)}
                style={{
                  cursor: "pointer", borderRadius: 8, overflow: "hidden",
                  border: `2px solid ${activeColor === chart.title ? "#f6b800" : "transparent"}`,
                  transition: "border-color .2s",
                }}
              >
                <img src={chart.image} alt={chart.alt} style={{ width: "100%", height: 70, objectFit: "cover" }} />
                <div style={{ padding: "6px 8px", background: "#111" }}>
                  <p style={{ margin: 0, fontSize: 11, fontWeight: 700, color: activeColor === chart.title ? "#f6b800" : "#ccc" }}>{chart.title}</p>
                </div>
              </div>
            ))}
          </div>
          {activeColor && (
            <div style={{ marginTop: 12, padding: "10px 14px", background: "rgba(246,184,0,.08)", borderRadius: 6, border: "1px solid rgba(246,184,0,.2)" }}>
              <p style={{ margin: 0, fontSize: 13, color: "#f6b800" }}>Selected: <strong>{activeColor}</strong> — Message us to lock this in!</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="ds-card" style={{ background: "#f6b800", border: "none" }}>
        <div className="ds-card-body" style={{ textAlign: "center", padding: "28px 24px" }}>
          ...
          <p style={{ margin: "0 0 16px", fontSize: 13, color: "rgba(0,0,0,.7)" }}>Available 7 days a week via WhatsApp.</p>
