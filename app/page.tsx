...
const systems = [
  { name: "Epoxy Flake Floors", className: "flake", copy: "Dense broadcast flake systems for garages, shops, utility rooms, and commercial spaces.", href: "#digital-bid" },
  { name: "Metallic Epoxy", className: "metallic", copy: "Luxury pearlescent movement for showrooms, salons, retail interiors, basements, bars, and lobbies.", href: "#digital-bid" },
  { name: "Polished Concrete", className: "polished", copy: "Densified, reflective concrete for retail, warehouses, showrooms, and long-life commercial floors.", href: "#digital-bid" },
  { name: "Stained Concrete", className: "stained", copy: "Mottled, translucent decorative concrete for patios, restaurants, interiors, and warm stone-tone looks.", href: "#digital-bid" },
  { name: "Countertops & Overlayments", className: "countertop", copy: "Concrete countertop coatings, microtoppings, resurfacing, and specialty decorative systems.", href: "#digital-bid" },
];

const colorFamilies = ["XPS vinyl flake blends", "XPS metallic pigments", "XPS solid epoxy base coats", "XPS concrete dye and stain colors", "XPS glitter additive colors"];
const dashboardTabs = [
...
const seoPages = ["garage epoxy floors", "metallic epoxy floors", "polished concrete floors", "stained concrete patios", "concrete countertops", "microtopping overlayments", "epoxy floor training", "XPS Xpress locations", "open an epoxy store", "epoxy startup package", "epoxy floor cost", "decorative concrete near me"];
...
export default function HomePage() {
  return (
    <main className="national-site" id="top">
      ...
      <section className="signal-strip" aria-label="XPS lead generation proof"><div><strong>70+</strong><span>XPS Xpress satellite locations</span></div><div><strong>XPS</strong><span>America's #1 Epoxy Super Store</span></div><div><strong>24 hr</strong><span>digital bid response target</span></div><div><strong>PWA</strong><span>branded app install path</span></div></section>
      ...
      <section className="app-band" id="digital-bid">
        ...
        <form className="digital-bid-form" action="mailto:leads@nationalepoxypros.com" method="post" encType="text/plain">
          ...
          <div className="form-grid two"><label>Color chart family<select name="colorChart">{colorFamilies.map((family) => <option key={family}>{family}</option>)}</select></label><label>Existing surface<select name="surface"><option>Bare concrete</option><option>Old coating needs removal</option><option>Cracks or spalling</option><option>Tile, VCT, carpet, or glue</option><option>Countertop or vertical surface</option></select></label></div>
      ...
      <section className="systems-section" id="systems"><div className="section-lead"><p className="eyebrow">Premium Finish Library</p><h2>Dedicated floor pages for search, education, and conversion.</h2><p>Each finish routes the customer toward a digital bid, the XPS visualizer, color charts, training, products, or business-package interest.</p></div><div className="system-grid premium">{systems.map((system) => <article key={system.name}><div className={`swatch ${system.className}`} /><h3>{system.name}</h3><p>{system.copy}</p><a href={system.href}>Start bid</a></article>)}</div></section>
      ...
      <section className="dashboards" id="dashboards"><div className="section-lead"><p className="eyebrow">App Dashboards</p><h2>One premium interface, four operational views.</h2></div><div className="dashboard-shell"><aside className="dashboard-nav" aria-label="Dashboard roles">{dashboardTabs.map(([role]) => <span key={role}>{role}</span>)}</aside><div className="dashboard-panel"><div className="dashboard-topline"><span>Owner Dashboard</span><strong>Lead engine active</strong></div><div className="dashboard-grid">{dashboardTabs.map(([role, detail]) => <div key={role}><span>{role}</span><strong>{detail}</strong></div>)}</div><div className="timeline"><span className="done">Lead captured</span><span className="active">Design review</span><span>Bid sent</span><span>Install scheduled</span></div></div></div></section>
      ...
      <section className="seo-directory"><div className="section-lead"><p className="eyebrow">SEO Expansion Scaffold</p><h2>64-page organic structure staged around the concrete coatings market.</h2></div><div className="page-grid">{seoPages.map((page) => <a key={page} href="#digital-bid">{page}</a>)}</div></section>
